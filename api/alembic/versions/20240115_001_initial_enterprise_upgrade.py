...
def upgrade() -> None:
    ...
    with op.batch_alter_table("utilities", schema=None) as batch_op:
        ...
        batch_op.create_index("ix_utilities_geo", ["latitude", "longitude"])

        # Index for category-based searches
        batch_op.create_index("ix_utilities_category", ["category"])

        # Index for verified utilities
        batch_op.create_index("ix_utilities_verified", ["verified"])
    ...
    op.create_table(
        ...
        sa.Column(
            "utility_id",
            sa.String(50),
            sa.ForeignKey("utilities.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "user_id",
