...
const UtilityDetailsComponent: React.FC<UtilityDetailsProps> = ({
  utility,
  onClose,
}) => {
  ...
  const handleShare = async () => {
    const typeName = getUtilityTypeName(utility.type || utility.category);
    const directionsUrl = `https://maps.google.com/?daddr=${utility.latitude},${utility.longitude}`;

    let message = `Check out ${utility.name} on UrbanAid!\n\n${typeName}`;
    if (utility.address) {
      message += `\n${utility.address}`;
    }
    ...
    try {
      await Share.share({ message, title: utility.name });
    } catch (error: any) {
  ...
  return (
    <Card mode="outlined">
      <Card.Content>
        ...
        {/* Transit Arrivals (only for transit stops) */}
        {utility.category === 'transit' && (
          <>
            <Divider style={styles.divider} />
            <TransitArrivals utilityId={utility.id} />
          </>
        )}
...
// Memoize to prevent flickering of Phoenix Sky Harbor popup
export const UtilityDetails = memo(UtilityDetailsComponent, (prevProps, nextProps) => {
  return (
    prevProps.utility.id === nextProps.utility.id &&
    prevProps.utility.name === nextProps.utility.name &&
    prevProps.utility.description === nextProps.utility.description &&
